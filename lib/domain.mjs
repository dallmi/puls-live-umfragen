...
/** Beschriftungen für den Excel-Export je Sprache (H31). */
function exLabels(lang) {
  const de = {
    ...
    option: 'Option', votes: 'Stimmen', share: 'Anteil', term: 'Begriff', count: 'Anzahl',
    name: 'Name', answer: 'Antwort', time: 'Zeitpunkt', value: 'Wert', points: 'Punkte',
    avgPerPerson: 'Ø pro Person', rank: 'Rang', avgPos: 'Ø-Position', borda: 'Borda-Punkte',
    correct: 'Richtig', question: 'Frage', participants: 'Teilnehmende', answers: 'Antworten',
    average: 'Durchschnitt', ratings: 'Bewertungen', questions: 'Fragen',
    truncAnswers: (n) => `Hinweis: Export auf ${n} Antworten begrenzt — es können weitere existieren.`,
    truncQuestions: (n) => `Hinweis: Export auf ${n} Fragen begrenzt — es können weitere existieren.`,
    leaderboard: 'Rangliste', leaderboardSub: 'Punkte über alle Quiz-Folien',
    overview: 'Übersicht', slide: 'Folie', type: 'Typ',
    exportedAt: (code, ts) => `Code ${code} — exportiert ${ts}`,
  };
  const en = {
    ...
    option: 'Option', votes: 'Votes', share: 'Share', term: 'Term', count: 'Count',
    name: 'Name', answer: 'Answer', time: 'Time', value: 'Value', points: 'Points',
    avgPerPerson: 'Avg per person', rank: 'Rank', avgPos: 'Avg position', borda: 'Borda points',
    correct: 'Correct', question: 'Question', participants: 'Participants', answers: 'Answers',
    average: 'Average', ratings: 'Ratings', questions: 'Questions',
    truncAnswers: (n) => `Note: export limited to ${n} answers — more may exist.`,
    truncQuestions: (n) => `Note: export limited to ${n} questions — more may exist.`,
    leaderboard: 'Leaderboard', leaderboardSub: 'Points across all quiz slides',
    overview: 'Overview', slide: 'Slide', type: 'Type',
    exportedAt: (code, ts) => `Code ${code} — exported ${ts}`,
  };
  return lang === 'en' ? en : de;
}

/** Ergebnisse einer Präsentation als Excel-Arbeitsmappe (ein Blatt pro Folie). `lang`: 'de'|'en' (H31). */
export function exportWorkbook(pres, lang) {
  const L = exLabels(lang);
  const sheets = [];
  const overviewData = [];
  pres.slides.forEach((slide, i) => {
    const results = computeResults(slide, !!pres.collectNames);
    const label = L.types[slide.type] || slide.type;
    ...
    switch (slide.type) {
      ...
      case 'open':
        sheet = pres.collectNames
          ? {
            headers: [L.name, L.answer, L.time],
            data: [...results.texts].reverse().map((t) => [t.name || '—', t.text, formatTs(t.ts)]),
            totals: [[L.answers, results.texts.length, ''], ...(results.texts.length >= 200 ? [[L.truncAnswers(200)]] : [])],
          }
          : {
            headers: [L.answer, L.time],
      ...
      case 'scale': {
        ...
        sheet = {
          headers: [L.value, L.count],
          data,
          totals: [
            [L.average, results.voters ? Math.round(results.avg * 100) / 100 : '–'],
            [L.ratings, results.voters],
          ],
        };
        break;
      }
      case 'points': {
        ...
        sheet = {
          headers: [L.option, L.points, L.avgPerPerson, L.share],
          data: (slide.options || []).map((opt, j) => {
      ...
      case 'ranking':
        sheet = {
          headers: [L.rank, L.option, L.avgPos, L.borda],
      ...
      case 'quiz': {
        const total = results.counts.reduce((a, b) => a + b, 0);
        sheet = {
          headers: [L.option, L.votes, L.share, L.correct],
          data: (slide.options || []).map((opt, j) => {
            const n = results.counts[j] || 0;
      ...
      case 'qa':
        sheet = pres.collectNames
          ? {
            headers: [L.name, L.question, L.votes, L.time],
            data: results.questions.map((q) => [q.name || '—', q.text, q.votes, formatTs(q.ts)]),
            totals: [[L.questions, results.questions.length, '', ''], ...(results.questions.length >= 100 ? [[L.truncQuestions(100)]] : [])],
          }
          : {
            headers: [L.question, L.votes, L.time],
  ...
  if ((pres.slides || []).some((s) => s.type === 'quiz')) {
    ...
    sheets.push({
      name: L.leaderboard,
      pre: [[L.leaderboard], [L.leaderboardSub]],
      headers: [L.rank, L.name, L.points, L.correct],
      data: board.map((r, j) => [j + 1, r.name, r.points, r.correct]),
      totals: [[L.participants, board.length, '', '']],
    });
  }
  sheets.unshift({
    name: L.overview,
    pre: [[pres.title], [L.exportedAt(pres.code, formatTs(Date.now()))]],
    headers: [L.slide, L.type, L.question, L.participants],
    data: overviewData,
    totals: [],
  });
